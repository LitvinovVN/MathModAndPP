// Задача 10. Организовать процесс конвейерного вычисления с тремя блоками на GPU.
// Номер шага конвейера r изменяется в диапазоне от 1 до 5 с паузой 1 секунда в функции CPU.
...
int main()
{
    ...
    cuda_kernel_function<<<BLOCK_DIM,1,BLOCK_DIM*sizeof(int)>>>(dev_counter);    // cuda_kernel_function starts running, динамическое выделение распределяемой памяти
