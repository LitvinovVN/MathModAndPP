...
{    
    ...
    auto start_chrono_file_r = std::chrono::high_resolution_clock::now();
    std::thread th_fr_1(fread_arr_double, file_name, N, data);
    th_fr_1.join();
    auto end_chrono_file_r = std::chrono::high_resolution_clock::now();    
    print_execution_time("Reading test", start_chrono_file_r, end_chrono_file_r, data_size_b);
...
int main()
{
    ...
    printf("--- Test 03---\n");
    fwrite_arr_double_test_1thread_1file(N, "test_3.bin");

    printf("--- Test 04---\n");
    fwrite_arr_double_test_1thread_1file(N, "C:\\data\\test_4.bin");
}