// Записать отчет в файл report.txt

#include <stdio.h>
#include <malloc.h>
#include <thread>

void print_execution_time(FILE *fp, const char* message, std::chrono::system_clock::time_point start,
...
{    
    ...
    {
        fprintf(fp, message);
        fprintf(fp, ": %ld bytes (%.4f mb); ", data_size_b, data_size_mb);
        fprintf(fp, "%.3lf mks;", elapsed_mks_double);    
        fprintf(fp, "speed: %.2f mb/sec\n", speed_mb_sek);
    }
}
...
void fwrite_arr_double_test_2threads_2files(FILE *fp, size_t N,
    const char* file_name_1, const char* file_name_2)
{
    ...
    print_execution_time(fp, "Writing test", start_chrono_file_w, end_chrono_file_w, data_size_b);    
}

// 1 массив записывается на диск 1 потоком в 1 файл
void fwrite_arr_double_test_1thread_1file(FILE *fp, size_t N,
...
// Считывает целое число из текстового файла file_name и возвращает его
size_t read_config(const char* file_name)
{
    size_t N = 0;
    ...
    if (N > 2140000000 || N < 0) N = 0;
    
    return N;
}

size_t read_command_line(int argc, char *argv[])
{    
    size_t N = 0;
    if(argc == 2)                          
        sscanf(argv[1], "%u", &N);    
...
{    
    ...
    // файл отчета
    const char * filename_report = "report.txt";
    // запись в файл
    FILE *fp = fopen(filename_report, "w");
                
    size_t N = 0;
    N = read_command_line(argc, argv);
    if (N == 0)
        N = read_config("config.txt");
    printf("N = %d\n", N);
    if (fp) fprintf(fp, "N = %d\n", N);

    printf("Test 01: Writing on same disk by 1 thread\n");
    fwrite_arr_double_test_1thread_1file(fp, N, "test_1.bin");

    printf("Test 02: Writing on other disk by 1 thread\n");
    fwrite_arr_double_test_1thread_1file(fp, N, "C:\\data\\test_2.bin");

    printf("Test 03: Writing on same disk by 2 threads on 2 files\n");
    fwrite_arr_double_test_2threads_2files(fp, N, "test_3_1.bin", "test_3_2.bin");

    printf("Test 04: Writing on other disk by 2 threads on 2 files\n");
    fwrite_arr_double_test_2threads_2files(fp, N, "C:\\data\\test_4_1.bin", "C:\\data\\test_4_2.bin");

    printf("Test 05: Writing on 2 different disks by 2 threads on 1 file\n");
    fwrite_arr_double_test_2threads_2files(fp, N, "test_5_1.bin", "C:\\data\\test_5_2.bin");

    if (fp) printf("Report has been written in %s\n", filename_report);
    fclose(fp);
}