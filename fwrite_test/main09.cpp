...
int main()
{
    ...
    auto elapsed_mks_file_w = std::chrono::duration_cast<std::chrono::microseconds>(end_chrono_file_w - start_chrono_file_w);
    double elapsed_mks = elapsed_mks_file_w.count();
    printf("Writing files: %.3lf mks\n", elapsed_mks);
    size_t data_size_b = 2 * N * sizeof(double); // 2 массива записываются на диск
    double data_size_mb = data_size_b / (1024.0*1024);    
    double write_speed_mb_sek = data_size_mb * 1000 * 1000 / elapsed_mks ;
    printf("data_size: %ld bytes (%f mb)\n", data_size_b, data_size_mb);
    printf("write_speed: %f mb/sec\n", write_speed_mb_sek);
    ...
    auto elapsed_mks_file_r = std::chrono::duration_cast<std::chrono::microseconds>(end_chrono_file_r - start_chrono_file_r);
    double elapsed_mks_r = elapsed_mks_file_r.count();
    printf("\nReading files: %.3lf mks\n", elapsed_mks_r);        
    double read_speed_mb_sek = data_size_mb * 1000 * 1000 / elapsed_mks_r ;
    printf("data_size: %ld bytes (%.4f mb)\n", data_size_b, data_size_mb);
